...
Download_BaiduPCS_Web(){
	...
	wget -N --no-check-certificate "https://github.com/liuzhuoling2011/baidupcs-web/releases/download/${BaiduPCS_Web_new_ver}/BaiduPCS-Go-${BaiduPCS_Web_new_ver}-linux-${bit}.zip"
...
Service_BaiduPCS_Web(){
	if [[ ${release} = "centos" ]]; then
		if ! wget --no-check-certificate https://raw.githubusercontent.com/iErics/baidupcsweb/master/BaiduPCSWeb_centos -O /etc/init.d/BaiduPCSWeb; then
		...
		if ! wget --no-check-certificate https://raw.githubusercontent.com/iErics/baidupcsweb/master/BaiduPCSWeb_debian -O /etc/init.d/BaiduPCSWeb; then
			echo -e "${Error} BaiduPCS-Web服务 管理脚本下载失败 !" && exit 1
		fi
		Download_BaiduPCS_port
		chmod +x /etc/init.d/BaiduPCSWeb
...
Download_BaiduPCS_port(){
	if ! wget --no-check-certificate https://raw.githubusercontent.com/iErics/baidupcsweb/master/port -O "${Folder}/port"; then
...
Update_Shell(){
	local latest_version=$(curl -H 'Cache-Control: no-cache' -s -L "https://raw.githubusercontent.com/iErics/baidupcsweb/master/BDW.sh" | grep 'sh_ver' -m1 | cut -d\" -f2)
	...
	if [[ $latest_version == $sh_ver ]]; then
		...
		echo
		wget -N --no-check-certificate "https://raw.githubusercontent.com/iErics/baidupcsweb/master/BDW.sh" && chmod +x BDW.sh
...
本脚本的项目地址：https://github.com/iErics/baidupcsweb
